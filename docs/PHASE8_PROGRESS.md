...
## Update — 2026-09-23: S001 static FVG defect audit; D002 blocked before execution; H005 registered; V001 preregistered (governance-only, no empirical read)

- **S001 (`STATIC_IMPLEMENTATION_AUDIT`, `Frozen FVG ATR Interface Incompatibility`):** `utils/indicators.py::calculate_atr` returns a scalar (`0.0` or `float(atr_series.iloc[-1])`), while `bot/analysis/fvg_engine.py::detect_fvgs` gates every iteration on `atr_series = atr if hasattr(atr, "iloc") else None` being non-`None`. A scalar has no `.iloc`, so `atr_series` is always `None` and the detector returns `[]` for every frame. Deterministic implementation/interface defect: `STATIC_SOURCE_PROOF — NOT EMPIRICAL MARKET EVIDENCE`. Bound to source commit `3d2cfd2496663118a4ffcacad4c7c1fe7b5bf814` with exact blob identities (`utils/indicators.py` `5324ae9e…`, `bot/analysis/fvg_engine.py` `ceec93a7…`, `bot/strategy/regime.py` `4ffa993f…`). No empirical read; no strategy execution; no budget consumed.
- **D002 → `BLOCKED_BEFORE_EXECUTION — DETERMINISTIC_FVG_ATR_INTERFACE_DEFECT`:** Stage-B attrition is deterministically fixed at zero by S001, so executing D002 under unchanged frozen semantics would spend a diagnostic without informing Stages C–G. Execution count zero; no budget consumed; no result record; no raw output. Specification retained unchanged (append-only disposition section added). D002 remains historically preregistered.
- **H004 → `SUPPORTED_BY_STATIC_IMPLEMENTATION_AUDIT` (narrow):** the D001 zero canonical FVG observation is explained by a deterministic implementation-stage defect and cannot be read as evidence that three-candle imbalance structure is absent from Fold 01. Nothing is established about real FVG counts, the 1.5× ATR rule, direction, fill status, OB overlap, candidate creation or profitability. Support covers pipeline causation of the canonical zero only.
- **H005 registered (OPEN):** `FVG ATR-Series Interface Repair` — the zero-output behavior is an interface mismatch; supplying a mathematically equivalent rolling ATR series (`bot.strategy.regime.atr_series`) while preserving all thresholds/filtering semantics should restore canonical FVG evaluation without introducing a new trading threshold.
- **V001 registered (`REGISTERED_NOT_IMPLEMENTED`):** `phase6-development-v2-V001` `Canonical FVG ATR-Series Compatibility Repair`; spec `docs/PHASE8_V2_VARIANT_V001.md` (SHA-256 `0b1d5ec57c68746f7c346c4b576220460d0dad54d218a2add34dbe6e2adbf8fb`). Change boundary: `utils.indicators.calculate_atr` unchanged (scalar public behavior preserved); local repair in the FVG detector path only; `atr_period=14`, `displacement_mult=1.5`, geometric/fill/direction/source-index/timeframe semantics preserved; no Gate-8/9, OB, confluence, overlap, session, news, DXY, risk, execution or broker changes. One-concept rule; ATR-series equivalence must be proven on multiple valid deterministic frames before empirical execution; two-phase Phase A freeze precedes any Fold-01 read.
- **Budget:** diagnostics `1 / 12 executed` (D001); D002 blocked before execution consumes nothing; strategy variants observed remain `0 / 8` (first empirical V001 result will consume `1 / 8` and trigger the upward-revision prohibition); numeric parameter trials remain `0 / 4` (V001 changes no numeric parameter); variant-budget result lock not yet triggered. No new empirical data read; Folds 02–04 and holdout untouched.

## Update — 2026-09-23: V001 empirical exposure and post-exposure measurement-tooling repair MR001 (no result record; rerun awaits authorization)

- **V001 empirical exposure occurred:** fresh causal Fold-01 rebuild under repaired V001 FVG semantics completed (budget-lock event `2026-09-23T15:23:28.661013+00:00`; strategy variants now `1 / 8` observed and `UPWARD STRATEGY-VARIANT / PARAMETER-TRIAL BUDGET REVISION PROHIBITED = ACTIVE` permanently). New immutable store `fold-01-a8b406884ab3525a` published (rows SHA-256 `a8b40688…`); historical `fold-01-1d710826193a6767` prohibited and untouched. Provenance phase pre-exposure (manifests/hashes/attestation/index) consumed nothing (§14 boundary).
- **Measurement blocked before any metric emission:** the frozen evaluator (`d87b7149…`) deterministically refused the conforming store with `BoundaryError: 2025+ evidence path refused` — a category error, not a boundary event: `run_v001_evaluation` stringified whole provenance mappings into the filesystem-path scanner `reject_holdout_path`, so hash-hex fragments of real SHA-256 digests (`e460066f4…` → `4600`, `8871`, …) were misread as future-year path components. Zero structural metrics were emitted. Blockage record (external, non-Git): `v2_variants/phase6-development-v2-V001/fold01/phase6-development-v2-V001_fold01_EXECUTION_BLOCKAGE.json`, SHA-256 `55a382dce5f00752e70ed03cb244db5f1277a471b78b43055819b3f118a03432`. Outcome at blockage: `EMPIRICAL_EXECUTION_BLOCKED`; H005 `INCONCLUSIVE_V001` (no claim either way).
- **MR001 (`POST_EXPOSURE_MEASUREMENT_TOOLING_REPAIR`, register record `phase6-development-v2-V001-MR001`):** repair restricted to provenance/path validation wiring in `backtests/phase8_v2_variant_v001_eval.py` — structured provenance validated by `_assert_source_dates_within_development` plus new field-scoped `_reject_holdout_identity_paths` (path scanning only for declared path-valued identity fields); `reject_holdout_path` itself unchanged and reserved for genuine paths. Metric surface, observer semantics, unique FVG identity, aggregation, density target, outcome classifications and H005 rules unchanged. Regression suite `tests/test_phase8_v2_mr001_boundary_scanner.py` proves the exact latent defect (real-length digests pass post-repair, fail under the historical wiring) while genuine holdout/`year=2025`/ISO-2025+ references still fail closed. Historical tooling commit `d87b7149…` remains immutable; its failure is part of the scientific record. No result record created; measurement rerun against `fold-01-a8b406884ab3525a` requires separate supervisory authorization.
- **Budget unchanged by the repair:** diagnostics `1 / 12 executed`; strategy variants `1 / 8 observed`; numeric parameter trials `0 / 4`; upward budget-revision lock ACTIVE. No store measurement or rebuild occurred during the repair; Folds 02–04, holdout and 2025+ untouched.


## Update — 2026-09-24: V001 Fold-01 structural result R001 published (measurement completed under MR003)

- **Measurement completed:** the preserved store `fold-01-a8b406884ab3525a` was measured with the MR003 tooling (`1a8e4b2…`) under separate supervisory authorization. Snapshot accounting reconciles exactly: 13,269 = 7,316 structural decisions + 4,815 missing_history + 1,138 unavailable_input + 0 evaluation_error. External deterministic payload `v2_variants/phase6-development-v2-V001/fold01/phase6-development-v2-V001_result.json` (SHA-256 `63e3be0a…`, 7,104 bytes, read-back verified) — full lineage, the three-blockage history (MR001–MR003) and all preregistered structural metrics are recorded in `docs/PHASE8_V2_VARIANT_V001_RESULT.md` and register record `phase6-development-v2-V001-R001`.
- **Structural outcome:** detector-eligible 1,489 with 1,489/1,489 exact observer reconciliation; ATR/FVG attrition is nonzero end-to-end (202,504 ATR-valid windows, 5,355 displacement passes, 2,275 bullish / 2,212 bearish geometric gaps, 705 unfilled zones); final canonical FVG presence 455 decisions / 579 zones / 65 unique identities; Gate-10 526 pass, Gate-11 1 pass / 525 fail; one canonical OB/FVG pair with overlap true; raw `candidate_ready` = 0 against target 90.
- **V001 outcome `FUNCTIONAL_REPAIR_NO_CANDIDATES`; H005 `SUPPORTED_BY_V001`:** the interface repair restores actual evaluation of the frozen canonical FVG semantics under unchanged thresholds; H005 support does not require candidates, the density target or profitability. No auto-promotion of H001/H002/H003/H004; no candidate selected or frozen.
- **No profitability metrics** were computed or emitted; no parameter counterfactuals were evaluated; budget unchanged: diagnostics `1 / 12`, strategy variants `1 / 8` (completed, not re-consumed), numeric trials `0 / 4`, upward-revision lock ACTIVE. Folds 02–04, holdout and 2025+ untouched; completing R001 does not authorize Tier B.


## Update — 2026-09-24: D003 preregistered (governance-only; no empirical read; zero budget)

- **D003 registered (`REGISTERED_NOT_EXECUTED`):** `phase8-v2-D003` — `Canonical Gate-11 and Order-Block Lifecycle Decomposition`; specification `docs/PHASE8_V2_DIAGNOSTIC_D003.md` (SHA-256 `2b520492…`); primary link `phase8-v2-H002` (statement unchanged after V001); H001 contextual-only (`INCONCLUSIVE_D001` unchanged); `H003 = NOT_TESTED_BY_D003`; H005 not reopened. Motivation: V001 R001 (Gate-11 526→1, candidate_ready 0, one OB/FVG pair with overlap true) — motivation only, never a test oracle.
- **Preregistered surface:** A accounting (D001 classification/reference-check/accounting reuse); B exact frozen score components (five checks, points, distribution, co-occurrence); C descriptive PD × legacy-OB × FVG-overlap contingency; D complete canonical `evaluate_order_block` BlockState/reason/side decomposition (eligibility = `ELIGIBLE` + `RETEST_ELIGIBLE` only); E acquisition-vs-canonical four-cell agreement; F canonical failure reasons ungrouped; G contextual FVG coexistence with the D001 mirror geometry; H premium/discount context. Canonical inputs reconstructed only from persisted causal snapshot data with reducer-exact `consumed_ids`; the lifecycle observer is read-only relative to strategy evaluation.
- **H002 decision rule fixed before execution:** `SUPPORTED_BY_D003` (lifecycle-failure concentration with rare canonical eligibility, argued structurally, no post-hoc percentage) / `NOT_SUPPORTED_BY_D003` (dominated by `no_confirmed_block` with little meaningful OB structure) / `INCONCLUSIVE_D003`. No strategy change follows automatically; no V002.
- **Prohibited:** all counterfactual scores/thresholds/condition-removals, temporal associations, profitability, and any encoding of observed V001 counts into tooling/tests. Future execution: `TIER_A_FOLD01` only, against preserved store `fold-01-a8b406884ab3525a`, consuming `2 / 12` upon first empirical execution. Registration consumed nothing; budget remains diagnostics `1 / 12`, variants `1 / 8`, numeric trials `0 / 4`; lock ACTIVE; Folds 02–04, holdout and 2025+ untouched.

## Update — 2026-09-25: D003 executed to a valid result (R001) after four pre-result tooling corrections

- **Lineage (all published, append-only):** preregistration `8b3a8bd…` → original tooling `882b4c9…` → **TC001** `aa6bbc1…` (observer receives the causal prior state record) → **TC002** `58bedf6…` (`canonical_git_blob_v1` committed-blob fingerprint) → **TC003** `b26ac6a…` (extraction from the frozen flattened D001 row + Gate-11 self-consistency/fail-closed invariants) → **TC004** `3e4514b…` (Surface-E semantic direction agreement). Every correction carries its own governance record; two post-exposure attempts were voided and preserved (`66373384…`/`764de0ea…` and `5c8c2a8b…`/`e28f976c…`), never overwritten.
- **Final rerun (TC004 tooling only, commit `3e4514b…`, fingerprint `c4a4d2f8…`):** `2026-09-25T10:44:13Z` → `10:47:05Z`, budget stays `2 / 12` (completion, not a new exposure). Sealed result `fold01/tc004-rerun1/phase8-v2-D003_result.json` SHA-256 `71f85982c3170c689678682184ccc2d3fe291bc74372a47e3dc8aed3a6d1b096` (5,595 bytes).
- **Result (all surfaces reconcile; 13,269 = 7,316 + 4,815 + 1,138 + 0; funnel 7,316 → 3,607 → 1,489 → 526 → 1 → 0; score distribution `{5: 522, 7: 3, 8: 1}`):** canonical lifecycle on the 526 Gate-11 entrants — ELIGIBLE 87 (LONG 50 / SHORT 37), EXPIRED 133, MITIGATED 28, INVALIDATED 3, UNAVAILABLE 275; reasons `no_confirmed_block 275`, `block_expired 133`, `block_already_mitigated 28`, `close_below_bullish_zone 3`, `confirmed_unmitigated_block 87`. Legacy-vs-canonical: four cells 437/85/2/2; semantic direction agreement 251 agree / 0 disagree / 275 not available. Coexistence: 60 canonical-eligible OB + final-FVG pairs, overlap true 39 / false 21.
- **H002 = `SUPPORTED_BY_D003`** (preregistered rule, this result only): 251/526 entrants carry a confirmed canonical OB-like block; 164 of those (65.3%) are consumed by later lifecycle failure concentrated in `block_expired` (133, dominant) plus mitigation (28) and invalidation (3); canonical eligibility is materially rarer (87, 16.5%) because of the lifecycle conjunction. The NOT_SUPPORTED arm fails because substantial structure demonstrably enters later lifecycle states. No numeric threshold invented; no strategy change follows automatically.
- **Boundaries unchanged:** H001 `INCONCLUSIVE_D001`, H003 `NOT_TESTED_BY_D003`, H005 `SUPPORTED_BY_V001`. No profitability metric, no counterfactual, no V002; Folds 02–04, holdout and 2025+ untouched. Budget: diagnostics `2 / 12`, variants `1 / 8`, numeric trials `0 / 4`; lock ACTIVE. Next supervisory decision: whether to preregister a V002 variant targeting the measured expiry/mitigation lifecycle bottleneck within the frozen variant budget.

## Update — 2026-09-25: D004 preregistered; V002 deferred — expired-block fate diagnostic

- **Supervisory V002 decision recorded: `DEFERRED — DO NOT CREATE V002 YET`.** A simple canonical-OB substitution variant is mathematically incapable of reaching the charter's Tier-A research target because D003 observes only 87 canonically eligible Gate-11 decisions (`candidate_ready <= Gate11 pass <= canonical eligible = 87 < 90`). No variant budget spent on a trial known in advance to be opportunity-insufficient.
- **H006 registered (`REGISTERED`):** `phase8-v2-H006` — `Fixed Order-Block Expiry May Preempt Structural Lifecycle Evidence`; class `HYPOTHESIS_NOT_CONCLUSION`; no claim that expiry should be removed, lengthened or parameterized. Failure condition registered: a high EXPIRED count alone does NOT support H006 — support weakens if expired blocks are mostly mitigated/invalidated/directionally inconsistent/stale. Disposition rule fixed BEFORE execution: `SUPPORTED_BY_D004` (meaningful proportion of EXPIRED blocks untouched + uninvalidated at decision; FVG coexistence strengthens but is not required; argued from concentration, no invented percentage cutoff) / `NOT_SUPPORTED_BY_D004` (predominantly touched/invalidated/stale) / `INCONCLUSIVE_D004` (sparse/unsafe decomposition).
- **D004 registered (`REGISTERED_NOT_EXECUTED`):** `phase8-v2-D004` — `Expired Order-Block Structural Fate Decomposition`; specification `docs/PHASE8_V2_DIAGNOSTIC_D004.md` (SHA-256 `cededb5f…`); primary link `phase8-v2-H006`; contextual prior `H002 = SUPPORTED_BY_D003` (NOT reopened). Primary population: Gate-11 entrants whose frozen canonical evaluation is `BlockState.EXPIRED` with a confirmed same-side block; reference populations ELIGIBLE/RETEST_ELIGIBLE/MITIGATED/INVALIDATED (UNAVAILABLE count-only). Read-only fate predicates (confirmation/candidate/decision timestamps, post-confirmation bars, bars beyond the frozen boundary, zone overlap + first-overlap index, invalidating close + first index, untouched-through-decision, first-retest-on-final-candle, side, zone); categories `EXPIRED_UNTOUCHED` / `EXPIRED_TOUCHED` / `EXPIRED_INVALIDATED` / `EXPIRED_FIRST_RETEST_AT_DECISION` as raw predicate counts (overlap allowed, none called eligible); exact age histogram only (no "would pass at expiry = X"); final FVG context from the persisted surface with the frozen D001 mirror geometry (no redetection); fate × FVG contingency; consumption invariant asserted (a returned-EXPIRED block cannot be consumed — fail closed); causality `available_at <= decision_at`; block-identity reconciliation with `evaluate_order_block` selection (fail closed); D003-semantics reconciliation (fail closed).
- **Frozen configuration:** `order_block_expiry_bars = 30` — unchanged, never tested at 31/40/45/60/90/no-expiry; no grid, no numeric trial. D004 does NOT test "would removing expiry create ≥90 candidates" and computes no alternate candidate/eligibility/Gate-11 surface. H001 `INCONCLUSIVE_D001`, H003 `NOT_TESTED_BY_D004`, H005 `SUPPORTED_BY_V001` — all unchanged. No profitability metric; no V002.
- **Budget:** registration and tooling consume nothing; current diagnostics `2 / 12`, variants `1 / 8`, numeric trials `0 / 4`; lock ACTIVE. A future separately authorized D004 empirical execution consumes `3 / 12` upon first store read. Store `fold-01-a8b406884ab3525a` NOT opened during preregistration; Folds 02–04, holdout and 2025+ untouched.

## Update — 2026-09-25: D004 executed to a valid result (R001) — H006 SUPPORTED

- **Lineage:** preregistration `e37b522…` → tooling freeze `b68d4e8…` (fingerprint `b71c2440…`) → single valid empirical run. Attempt 1 (13:51:26Z) was a VOID driver-preflight blockage (mis-specified identity assertion in the untracked execution driver; zero observations; frozen tooling never began; nothing written) — sealed as `phase8-v2-D004_ATTEMPT1_BLOCKAGE.json` (SHA-256 `ad789aea…`), preserved intact. Valid run: 13:53:24Z → 13:56:20Z (176 s) after store identity digest + rows SHA verified immediately before observation.
- **Budget exposure:** the first store load permanently consumed the third diagnostic — diagnostics now `3 / 12`, variants `1 / 8`, numeric trials `0 / 4`; lock ACTIVE. The attempt-1 blockage consumed nothing additional.
- **Result (all preregistered reconciliations pass):** accounting `13,269 = 7,316 + 4,815 + 1,138 + 0`; funnel `7,316 → 3,607 → 1,489 → 526 → 1 → 0` (identical to D003 — read-only observer); Gate-11 entrants 526 partition exactly EXPIRED 133 + ELIGIBLE 87 + MITIGATED 28 + INVALIDATED 3 + UNAVAILABLE 275. EXPIRED fate predicates (raw, overlap allowed): untouched 57, zone-overlapped 76 (49 invalidating-close, 27 mitigation-like touches), first-retest-on-final-candle 0; block identity/zone/side reconciled for all 133; consumption invariant zero violations; age min 31 / median 68 / max 135 (p25 41, p75 90), exact histogram sums to 133. FVG context (persisted surface, frozen mirror geometry): 55/133 with final FVG, direction agree 55 / disagree 0 / not_available 78, canonical overlap true 22 / false 33; 22 untouched blocks retain a same-direction FVG with canonical overlap true.
- **H006 = `SUPPORTED_BY_D004`** (preregistered rule, no numeric cutoff invented): 57/133 (42.9%, the modal single fate) of frozen EXPIRED blocks remain structurally untouched AND uninvalidated at the causal decision — age is their only adverse lifecycle evidence — and 42 of those 57 retain same-direction final FVG structure (H006's registered strengthening). The NOT_SUPPORTED arm's "predominantly stale" does not hold over the modal untouched fate; counterweights recorded honestly (76 blocks did interact with price; 78 lack FVG context). No strategy change follows automatically.
- **Boundaries unchanged:** H001 `INCONCLUSIVE_D001`, H002 `SUPPORTED_BY_D003` (not reopened), H003 `NOT_TESTED_BY_D003`, H005 `SUPPORTED_BY_V001`. No alternate expiry evaluated; no candidate counterfactual; no profitability metric; no V002. Sealed result `fold01/phase8-v2-D004_result.json` SHA-256 `6563a93d426045df5ab48d0554d96deff1b9b82132071192be815e46cd723c9d` (6,984 bytes). Folds 02–04, holdout and 2025+ untouched. Next: supervisory interpretation of D004 R001 decides any V002.

## Update — 2026-09-25: H001 evidence synthesis published; V002 preregistered (governance-only, no empirical read)

- **H001 = `SUPPORTED_BY_D003_D004_SYNTHESIS`.** Record `phase8-v2-H001-R001` (type `EVIDENCE_SYNTHESIS`, class `DEVELOPMENT_EVIDENCE_SYNTHESIS — D003+D004 R001 — NO NEW EMPIRICAL EXECUTION`) appended to the register from sealed D003+D004 R001 evidence only: no Fold-01 store read, no new replay, ZERO diagnostic budget (`3 / 12` unchanged, no D005). H001's registered statement/rationale/criteria untouched. Synthesis (structural pair observations, NOT candidate counts): D003 eligible-OB+final-FVG same-direction pairs 60 (overlap 39 true / 21 false) + D004 EXPIRED_UNTOUCHED-with-same-direction-FVG 42 (overlap 22 true / 20 false) = 102 same-direction structurally-intact pairs, 61 exact overlaps, 41 non-overlapping (populations lifecycle-disjoint). Preregistered qualitative criteria observed: pair availability not near-zero; coexistence materially exceeds exact overlap (102 vs 61); 41/102 coherent pairs fail exact overlap — exact overlap removes a substantial part of the pair surface. The registered failure condition does not hold. No new numeric acceptance threshold. Futility arguments recorded: expiry-only/OB-only preserving exact overlap → `candidate_ready <= Gate-11 passes <= 61 < 90`; overlap-only retaining legacy OB (passes 4/526) → futile.
- **H007 registered (`REGISTERED`):** `phase8-v2-H007` — `Canonical OB/FVG Structural-Pair Semantics`; class `HYPOTHESIS_NOT_CONCLUSION`; registered AFTER D003/D004 evidence with disclosure (`registered_after_evidence_was_observed: true`). One coherent canonical OB/FVG pair concept instead of the conjunction of legacy `ob_result.valid` + mandatory exact overlap: (1) confirmed same-side structurally-active canonical OB at the causal decision + (2) same-direction final canonical FVG. Exact overlap stays observable but is not mandatory.
- **V002 registered (`REGISTERED_NOT_OBSERVED`):** `phase6-development-v2-V002` — `Canonical Structural OB/FVG Pair`; linked `phase8-v2-H007`; supporting H002+H006+H001; spec `docs/PHASE8_V2_VARIANT_V002.md` (SHA-256 placeholder at preregistration, replaced at the freeze commit). Variant-specific evaluator: structural-active OB = canonical same-side confirmed + causally available + not consumed + no invalidating close + not already mitigated + first-retest consistent with canonical semantics; **age > 30 bars ALONE must NOT reject an otherwise untouched/uninvalidated block** (age recorded as metadata; global `order_block_expiry_bars = 30` and the historical canonical evaluator unchanged). FVG association: frozen final canonical FVG, causally available, same direction; exact overlap NOT mandatory but computed/recorded descriptively; no proximity/partial/temporal-lag rules. Confluence 2/1/2/1/2 max 8 threshold 8/8 unchanged; V002's 2-point component = canonical structurally-active OB present, 1-point = same-direction final canonical FVG associated with that OB (NOT labeled exact overlap). Downstream canonical-strategy decision must consume the SAME V002 semantics (no Gate-11 pass then age-only EXPIRED rejection of the same block); isolated V002 adapter path allowed. DXY unchanged and authoritative. Success classification preregistered: `OPPORTUNITY_SUFFICIENT` (candidate_ready >= 90, continued-research-only) / `OPPORTUNITY_INSUFFICIENT` (0 < r < 90) / `NO_CANDIDATES` (0) / `IMPLEMENTATION_FAILED` (semantics not functioning). 102/61/41/90 NOT encoded as test outputs (synthetic fixtures only).
- **Budget:** registration and synthesis consume nothing — diagnostics `3 / 12`, variants `1 / 8` observed (V002 becomes `2 / 8` only at first empirical observation), numeric trials `0 / 4`; lock ACTIVE. Phase A commit is governance-only (no strategy code); Phase B (implementation + synthetic tests + frozen Fold-01 structural measurement tooling, never executed here) follows only after this commit is pushed and remote-verified; HARD STOP before any V002 empirical execution. Folds 02–04, holdout and 2025+ untouched.

## Update — 2026-09-25: V002 implemented, synthetic-tested and frozen — NO empirical execution

- **Phase A lineage:** governance commit `3d910bec…` (parent `cd752626…`) pushed and remote-verified: register `phase8-v2-H001-R001` synthesis + H001 flip + H007 + V002, spec `docs/PHASE8_V2_VARIANT_V002.md` (SHA-256 `f53f27c4…`), progress/handoff.
- **Implementation (`feat: freeze V002 canonical structural pair`, parent = the Phase-A commit):** `bot/strategy/variant_v002.py` — isolated V002 pair evaluator (`evaluate_v002_structural_pair`: canonical detection/selection/causality/consumed/invalidation/mitigation semantics verbatim, the age-only EXPIRED short-circuit replaced by metadata `age_bars`), V002 Gate-11 scorer (`score_v002_setup`, frozen 2/1/2/1/2 weights, max 8, threshold 8/8, V002 component labels — no exact-overlap labeling) and `evaluate_v002_strategy` downstream path consuming the SAME pair semantics (canonical `evaluate_legacy_context`/engine untouched; `StrategyConfig` fingerprint unchanged). Historical V1/V001 EXPIRED behavior proven preserved on the identical fixture frames.
- **Synthetic tests:** `tests/test_phase8_v2_v002_variant.py` (17) + `tests/test_phase8_v2_v002_downstream.py` (8) — full §21 invariant list incl. the age-alone divergence proof (>30-bar untouched block ACTIVE under V002 vs EXPIRED canonical on the same frame), LONG/SHORT, overlap-true/overlap-false acceptance, no-FVG rejection, Gate 11 8/8, DXY/news/session/allowlist invariance. Tooling tests `tests/test_phase8_v2_v002_eval_tooling.py` (13): boundaries, historical-store refusal, store-compatibility fail-closed, full mocked-store loop, banned metrics, `canonical_git_blob_v1` provenance, non-overwriting output.
- **Frozen measurement tooling:** `backtests/phase8_v2_variant_v002_eval.py` — D001 loop discipline verbatim; V002 observer on Gate-11 passers only; preregistered structural surface only (no profitability metric, banned-metric guard); store-compatibility decision **B — preserved V001 store semantically sufficient** (all V002 causal inputs reconstructable from existing snapshots; asserted per snapshot, fail closed); refuses historical store, Tier B, holdout, 2025+; non-overwriting content-hashed output. **NOT executed** — zero Fold-01 reads in this task.
- **Full repository suite (strategy semantics change): 1,662 passed, 1 skipped (pre-existing opt-in keyring skip), 10 subtests, zero failures (44m45s); focused batteries 375 passed; `git diff --check` clean; secret scan clean; spec doc byte-identical to Phase A (SHA `f53f27c4…` now bound in the register, placeholder removed).**
- **Budget unchanged:** diagnostics `3 / 12`, variants `1 / 8` observed, numeric trials `0 / 4`; lock ACTIVE. V002 becomes `2 / 8` only at the instant first Fold-01 V002 strategy behavior is empirically observed. HARD STOP — Fold-01 empirical execution awaits separate supervisory authorization; Tier B remains sealed until `candidate_ready >= 90` on Fold 01.

## Update — 2026-09-25: V002 TC001 — Gate-11 entrant population correction (pre-empirical, tooling only)

- **Defect class `V002_PRE_EMPIRICAL_GATE11_PASSER_POPULATION_MISMATCH` (found in supervisory review, before any empirical access):** the frozen V002 tooling at `4890dcb…` selected its population from historical Gate-11 PASSERS (`bool(gate_results["gate_11_confluence_score"])`) and raised on `legacy_score_passed == False`. Since V002 changes the Gate-11 OB/FVG structural semantics, a decision the old semantics rejected is exactly one V002 may legitimately accept — the old Gate-11 boolean was acting as an eligibility filter for the new strategy, blocking measurement of V002's own semantic effect. Governance record `phase6-development-v2-V002-TC001` (`PRE_EMPIRICAL_MEASUREMENT_TOOLING_CORRECTION`) appended to the register.
- **Correction (measurement population ONLY):** V002 population = every decision that legitimately ENTERED frozen Gate 11 (`"gate_11_confluence_score" in gate_results` — upstream Gates 8/9/10 unchanged and not broadened); the historical Gate-11 boolean and `legacy_canonical_fvg_in_ob` are descriptive comparison metadata only (legacy pass/fail counts added to the aggregate). New fail-closed reconciliations: `gate11_entrants_observed == funnel entered` and `candidate_ready <= v002 score passes <= entrants`. Regressions added: real-adapter legacy-fail entrant observed, V002-rescue semantic difference observable (legacy fail → V002 8/8 pass → strategy eligible), mixed-population loop (non-entrant excluded; fail + pass entrants both observed), and old-bug reproduction against the byte-identical defective tooling blob (SHA `fd5629f7…`) proving both the rejection and skip arms.
- **Invariants:** `bot/strategy/variant_v002.py` unchanged (0-byte diff); H001/H007/V002 preregistration and spec (SHA `f53f27c4…`) untouched; success classification 90 threshold unchanged; store compatibility stays **B — semantically sufficient**; zero Fold-01 reads, no replay, no Tier B; budget unchanged `3 / 12`, `1 / 8`, `0 / 4`, lock ACTIVE. Commit `fix: observe all V002 Gate-11 entrants` (parent `4890dcb…`); HARD STOP — corrected tooling awaits supervisory review before any V002 empirical execution.

## Update — 2026-09-25: V002 TC002 — candidate_ready corrected through frozen Gates 12/13 (pre-empirical, tooling only)

- **Defect class `V002_PRE_EMPIRICAL_CANDIDATE_READY_STAGE_MISMATCH` (found in supervisory review of TC001, before any empirical access):** the TC001-corrected tooling counted `evaluate_v002_strategy(...).entry_eligible` as `candidate_ready` — the canonical-strategy-equivalent stage only. Frozen V1/V001 semantics additionally require Gates 12–13 via `strategies.smc_engine.entry_model.determine_entry` before the reducer emits `action = candidate_ready`, so the raw-candidate stage measured against the charter's `candidate_ready >= 90` was wrong. Governance record `phase6-development-v2-V002-TC002` (`PRE_EMPIRICAL_MEASUREMENT_TOOLING_CORRECTION`) appended to the register; TC001 remains accepted and the V002 population remains ALL Gate-11 entrants.
- **Correction (measurement staging ONLY):** explicit V002 variant funnel `gate_11_v002` (entrants; V002 8/8 pass) → `canonical_strategy_v002` (entry_eligible) → `gate_12_13_rr_entry_v002` (frozen `determine_entry` non-None); `candidate_ready = count(v002_entry_ready)` — upstream Gates 8–10 + V002 Gate 11 + V002 canonical strategy + frozen Gate-12/13. The frozen entry model is reused directly (no new entry model; V002 score passed as `score_result`; only the H007 structural context fields `ob_zone`/`fvg_zone`/`htf_zone_alignment` carry V002 meaning — the preregistered downstream-consistency consequence of H007). Entry runs over a PRIVATE canonical `state_from_record` reconstruction of the decision's own `next_record`; the historical D001 state chain is never mutated; prior-state consumption for the V002 pair remains `prior_state_record`-only. Candidate setup IDs are the reducer's persisted stable setup identities (event-id reconciled, fail-closed), with decision IDs kept separately descriptive; unique + duplicate occurrence counts reconcile to `candidate_ready` (never silently deduplicated).
- **Hard reconciliations (fail closed):** `candidate_ready == gate_12_13_rr_entry_v002.passed`; chain `candidate_ready ≤ strategy pass ≤ Gate-11 pass ≤ entrants`; funnel entered-chains; setup-ID reconciliation; TC001's `entrants == funnel entered` retained. Regressions: strategy-eligible/entry-not-ready NOT counted (old-bug reproduced against the byte-identical `c2e7409…` fixture, SHA `62a4dbaa…`); full-candidate counts once; legacy-equivalence reproduces the historical reducer when semantics coincide and does not rescue historical `entry_not_ready`; funnel 4→3→2→1→1; candidate-ID/identity/mismatch fail-closed.
- **Store compatibility reaffirmed B:** all Gate-12/13 causal inputs (entry price from persisted `entry_rows`, session context, liquidity pools, internal structure, sweep signal) exist in the persisted payload/state reconstruction; `REQUIRED_PAYLOAD_FIELDS` extended and asserted per snapshot fail-closed; no missing input, no rebuild.
- **Invariants:** `bot/strategy/variant_v002.py` byte-identical (blob `8272c285…`); H007/spec scientific content unchanged; 90 threshold unchanged; zero Fold-01 reads/no replay/no Tier B; budget `3 / 12`, `1 / 8`, `0 / 4`, lock ACTIVE. Commit `fix: measure V002 candidate readiness through entry gates` (parent `c2e7409…`); HARD STOP — corrected tooling awaits supervisory review before any V002 empirical execution.

## Update — 2026-09-25: V002 executed on Tier-A Fold 01 — R001 published — OPPORTUNITY_INSUFFICIENT (candidate_ready 61 < 90)

- **First empirical V002 observation (supervisory-authorized).** Exposure `2026-09-25T21:44:00.712191Z`; single valid run 239.5 s; strategy variants permanently **`2 / 8` observed**. Store `fold-01-a8b406884ab3525a` re-hashed (`verify_rows=True`, 13,269 rows; identity `a8b40688…`, rows `96c522e0…`) before any observation; implementation blob `8272c285…` verified equal at freeze and baseline; TC002 tooling fingerprint recomputed from the committed blob (`2151c217…`). Sealed result `v2_variants/phase6-development-v2-V002/fold01/phase6-development-v2-V002_result.json` SHA-256 `5addbb88e7885d8a604766d58e35a830478f519287d8e8a499ac219906b27a71` (12,912 bytes, read-back verified). Governance record `phase6-development-v2-V002-R001` appended; V002 status → `OPPORTUNITY_INSUFFICIENT_TIER_A_FOLD01_OBSERVED`.
- **Result (all hard reconciliations passed):** accounting `13,269 = 7,316 + 4,815 + 1,138 + 0`; historical funnel `7,316 → 3,607 → 1,489 → 526 → 1` (identical to D003/D004 — unchanged upstream). TC001 population 526 == funnel entered 526 (legacy pass 1 / fail 525, both observed). V002 pair surface: ACTIVE 144 / MITIGATED 92 / INVALIDATED 15 / UNAVAILABLE 275; same-direction FVG associated 110; exact overlap descriptive 67 true / 43 false (39.1% fail); active ages p25 5.8 / median 13.5 / p75 41.2 / max 113. V002 funnel: Gate-11 pass **102**/526 → canonical-strategy pass **80** → Gate-12/13 pass **61** = `candidate_ready 61` (rate 0.116; LONG 41 / SHORT 20; 61 unique setup IDs, 0 duplicates; candidate exact overlap 25 true / 36 false).
- **Classification: `OPPORTUNITY_INSUFFICIENT`** (0 < 61 < 90, mechanical). The frozen 102/61/41 synthesis figures were motivation only; the empirical 102/61 are independently derived and were never encoded as expectations. No profitability metric; no counterfactual (no alternate score/expiry/DXY/Gate-12/13 configuration); no V003. Tier B remains sealed — the preregistered Tier-B precondition (`candidate_ready ≥ 90` on Fold 01) is NOT met. Folds 02–04, holdout, 2025+ untouched. Budget: `3 / 12`, `2 / 8`, `0 / 4`, lock ACTIVE.

## Update — 2026-09-25: D005 preregistered — Temporal OB/FVG Association and Final-Surface Attrition (no empirical access)

- **Supervisory review of V002 R001 accepted; next authorized work is a READ-ONLY diagnostic.** Tier B denied (`61 < 90`, Fold 02 sealed). Downstream-only futility bounds recorded as supervisory arithmetic (NOT counterfactuals): best-case recovery of all 22 canonical-strategy rejects → `61+22=83 < 90`; best-case recovery of all 19 Gate-12/13 rejects → `61+19=80 < 90`; variants touching ONLY canonical strategy or ONLY Gate-12/13 are mathematically unable to reach 90 and are NOT created.
- **§5 compatibility gate PASSED before any D005 record was created:** the existing `phase8-v2-H003` (registered 2026-09-21, statement/rationale/expected effects/failure mode read in full and preserved byte-identically) genuinely concerns the temporal/sequential OB↔FVG relationship — its `implementation_boundary` reserves exactly this "later diagnostic" for explicit temporal-distance measurement and its `metrics_permitted` names "OB-to-FVG temporal distance distributions (later diagnostic only)" on `TIER_A_FOLD01`. D005 binds to H003 as preregistered; H003 is NOT rewritten.
- **D005 = `phase8-v2-D005` "Temporal OB/FVG Association and Final-Surface Attrition", `REGISTERED_NOT_EXECUTED`.** Primary population (structural definition, derived naturally at execution): V002 Gate-11 entrants with frozen V002 `structurally_active == True` AND `fvg_associated == False`. Reference populations R1/R2/R3 must partition the entrant universe. Causal FVG history through `decision_at` only; final-FVG recomputation must reconcile exactly with the persisted surface (fail closed); temporal universe distinguishes before/after OB, same/opposite direction, unfilled/filled; attrition categories `NO_SAME_DIRECTION_FVG_EVER` / `SAME_DIRECTION_FVG_EXISTED_BUT_FILLED` / `SAME_DIRECTION_FVG_PRE_OB_ONLY` / `OPPOSITE_DIRECTION_ONLY` (exhaustive, mutually exclusive, fail-closed). H003 temporal rule = preregistered ordering-based temporal sequence (same-direction canonical FVG formed at/after OB confirmation; fill state is descriptive universe metadata, NOT part of the ordering rule); no proximity/lag/window invented; no temporal parameter search; counterfactual-candidate concepts banned fail-closed.
- **Pre-empirical transcription defect corrected before tooling freeze (disclosed):** the initial Phase-A wording (spec SHA `bab242de…`, commit `df19e89…`) transcribed an extra "and is still unfilled at the decision timestamp" clause into the H003 rule. Store semantics (`build_gate_inputs` persists `get_unfilled_fvgs(entry, "M5", direction=htf_bias)` over the whole causal frame) make that clause vacuous — a primary-population decision (empty final surface) by definition has no same-direction unfilled FVG anywhere in the frame, so the rule would have forced `h003_temporal_association_decisions = 0`. Corrected to the ordering-only rule H003 actually preregisters: spec SHA `bab242de…` → `d7f39d31…` (§9 records the store-semantics rationale; §11 categories note primary-population post-OB FVGs are necessarily filled); register entry corrected byte-surgically (+934 bytes, D004/H003 equality validated, defect note recorded inside the entry). H003 statement untouched; zero Fold-01 access; no tooling had referenced the defective wording.

## Update — 2026-09-25: D005 Phase-B tooling frozen (synthetic-tested; HARD STOP before execution)

- **`backtests/phase8_v2_diagnostic_d005.py` implemented** (read-only; reuses D001 snapshot classification/reference-check/accounting, the frozen TC001+TC002 V002 observation path verbatim, canonical `detect_fvgs`/`get_unfilled_fvgs`/`detect_order_blocks`, `canonical_git_blob_v1` provenance and the Fold-01 boundary guards). Per-decision observation: causal-frame extraction (non-causal rows excluded and counted, never inspected), EXACT final-surface reconciliation against the persisted surface (fail closed), temporal-universe derivation from the frozen detector without the direction filter, block-identity reconciliation against the V002 observation (block_id + side; legacy `ob_result` zone agreement recorded descriptively only — the legacy detector is a different detector by design per H002), ordering-only H003 rule, exhaustive four-category attrition accounting, signed/absolute bar distance, elapsed minutes and formation-order/fill-state records. Aggregate reconciles the R1/R2/R3 partition against the V002 surface and applies the preregistered 29-observation headroom arithmetic (necessary condition only). Banned concepts (candidate_if_temporal, rescued_candidate, alternate/optimal lag, temporal_threshold, pnl/profit/win_rate/expectancy/drawdown/sharpe/closed_trades) rejected structurally at observation AND document level (frozen V002 banned list reused; negation markers exempted deliberately and their values still scanned).
- **`tests/test_phase8_v2_d005_tooling.py`: 28 synthetic tests** covering the required cases (no-FVG-ever; post-OB same-direction FVG = H003 temporal; pre-OB-only; formed-then-filled with fill state descriptive; unfilled-final-surface reference case refused as R1; opposite-direction-only; LONG/SHORT; post-decision FVG excluded causally and never counted; final-surface reconciliation exact/mismatch fail-closed; block-identity mismatch fail-closed; malformed timestamps fail closed; banned concepts; distance arithmetic; partition/headroom/category reconciliations incl. the 29 boundary; wrong-fold refusal; non-overwriting output). All fixtures valid-OHLC synthetic frames through the REAL canonical detectors — no empirical count encoded anywhere.
- **Invariants at freeze:** `bot/strategy/variant_v002.py` byte-identical (blob `8272c285…`); `backtests/phase8_v2_variant_v002_eval.py` unmodified; TC001/TC002 untouched; H003 statement byte-identical; zero Fold-01 reads, no replay, no Tier B. Budget `3 / 12`, `2 / 8`, `0 / 4`, lock ACTIVE — D005 becomes `4 / 12` only when a separately authorized execution first reads Fold 01. V003 = NOT CREATED. Full repository suite run before the freeze commit; HARD STOP after push — tooling awaits supervisory review.
- **Necessary headroom arithmetic (not science):** required headroom `90 − 61 = 29`; `TEMPORAL_VARIANT_HEADROOM_POSSIBLE` iff `h003_temporal_association_decisions ≥ 29`, else `INSUFFICIENT`; necessary only, no automatic V003. H003 disposition follows H003's own preregistered qualitative effect/failure mode via the register vocabulary (`SUPPORTED_BY_D005` / `NOT_SUPPORTED_BY_D005`), never the 29 rule.
- **Phase A commit:** spec `docs/PHASE8_V2_DIAGNOSTIC_D005.md` (SHA-256 `bab242de848a79714828d958196b830d8ae975445f7183c8c6ac03874a747401`, bound in the register); register append byte-surgical (+10,939 bytes, zero deletions; byte-recovery, D004-entry and H003 equality validated). Zero Fold-01 reads; budget stays `3 / 12`, `2 / 8`, `0 / 4`; V003 NOT CREATED. Phase B tooling + freeze next, HARD STOP before execution.

## Update — 2026-09-26: D005 TC001 pre-empirical temporal-clock correction (frozen; HARD STOP before execution)

- **Defect (registered `phase8-v2-D005-TC001`, class `D005_PRE_EMPIRICAL_TEMPORAL_CLOCK_MISMATCH`):** the frozen `81c6a6f` D005 tooling timestamped FVG formation with the completion candle's `open_time` while the OB clock is the confirmation candle's `available_at` (`block.confirmed_at`); on the contiguous M5 grid (`available_at(i) == open_time(i+1)`) every FVG was mislabeled exactly one bar early (true 0 → old −1 BEFORE; true +1 → old 0 AT). **Corrected clock:** `fvg_formation_available_at = causal_frame.iloc[source_index + 1]["available_at"]` — the same causal clock as the OB `confirmed_at`; the completion row stays `source_index + 1` (detector attribution untouched); the completion `open_time` is retained only as descriptive `fvg_completion_open_time` metadata with proven zero consumers (order category, signed/absolute distance and H003 membership are computed from the formation `available_at` alone). No manual ±1 compensation; the M5 bar-grid invariant (fail-closed) is unchanged.
- **Regressions added (all fixtures verified against the REAL canonical detectors; no empirical count):** same-candle (AT / 0; old tooling reports BEFORE / −1 and flips H003 membership to False), next-candle (AFTER / +1 / 5 min; old reports AT / 0), previous-candle (BEFORE / −1 / −5 min — correctly absent from the qualifying surface, descriptive in the universe), formation-field naming/causality, grid-invariant fail-closed, open-time descriptive invariance, and the in-process old-bug reproduction against the byte-identical committed defective fixture (`tests/fixtures/d005_tooling_81c6a6f.py`, SHA-256 `b8bc31fa…`; opt-in `D005_TC001_VERIFY_COMMITTED_BYTES=1` byte-identity check mirrors the V002-TC001 precedent). D005 suite now 35 passed + 1 opt-in skip.
- **Spec append-only §21 "TC001 — Causal Formation Clock Clarification"** — sections 1–20 and the original D005 scientific registration NOT rewritten; historical hashes preserved and verified from the committed Git blobs (`bab242de848a79714828d958196b830d8ae975445f7183c8c6ac03874a747401` at df19e89; `d7f39d31…` at c615c3e); corrected full-document SHA `2b820d608b1d881720369b045366e1986c9cf07f25ce44a2bba26ce837029ed8` bound in the tooling `SPEC_SHA256` and the register TC001 record. H003 statement byte-identical; ordering-only rule, primary population, 29-headroom arithmetic, final-surface reconciliation and attrition category definitions unchanged.
- **Governance:** register `result_records` closes with the byte-surgical additive `phase8-v2-D005-TC001` record (+7,249 bytes, zero deletions; byte-recovery, prior-record-equality and H003-statement-equality validated). Budget unchanged `3 / 12`, `2 / 8`, `0 / 4`, lock ACTIVE; V003 NOT CREATED; zero Fold-01 reads; `bot/strategy/variant_v002.py` (blob `8272c285…`) and `backtests/phase8_v2_variant_v002_eval.py` untouched. Corrected tooling awaits supervisory review — D005 NOT executed.
## Update — 2026-09-26: D005 Attempt 1 blocked at aggregation; TC002 partition + CLI boundary correction (zero-budget; HARD STOP before rerun)

- **D005 Attempt 1 executed and failed CLOSED after full empirical exposure (register `phase8-v2-D005-TC002`, `POST_EXPOSURE_MEASUREMENT_TOOLING_CORRECTION`).** Exposure (first authorized store processing) `2026-09-26T13:18:42.145570+00:00` — diagnostics permanently `4 / 12` (`2 / 8`, `0 / 4`, lock ACTIVE). The frozen `7c9608d5` tooling completed the full 13,269-snapshot observation loop over `fold-01-a8b406884ab3525a` (identity + rows re-hashed `verify_rows=True`) and `reconcile_accounting` passed, then failed at aggregation: `ReconciliationError: primary population 42 != structurally-active without final FVG R2=34`. No result was sealed (external blockage artifact `phase8-v2-D005_ATTEMPT1_BLOCKAGE.json`, SHA-256 `168ea3b27eb5cf536cca338bd87de028e2da2abd055ed112e85c508e1886f6e2`, 3,692 bytes, preserved unmodified); no H003 interpretation was made; V003 NOT created. A separate PRE-ACCESS wrapper refusal (substring-year scan misreading hash fragments `4068`/`3525` in the store basename) occurred before any store access and consumed zero exposure/budget.
- **Primary defect (`D005_V002_MARGINAL_INTERSECTION_PARTITION_MISMATCH`):** the old aggregator derived `R1 = associated`, `R2 = active − associated` from the frozen V002 aggregate MARGINALS — invalid, because `fvg_associated` is NOT a subset of `structurally_active` (a MITIGATED/INVALIDATED/CONSUMED block still carries its same-direction final FVG association; proven structurally by a new real-`evaluate_v002_structural_pair` regression on synthetic frames). **Corrected (TC002):** the partition is now the four-cell contingency (A `ACTIVE_ASSOCIATED` / B `ACTIVE_NOT_ASSOCIATED` = primary / C `NONACTIVE_ASSOCIATED` / D `NONACTIVE_NOT_ASSOCIATED`) built from individual per-decision V002 observations; R1 = A, R2 = B, R3 = C + D; marginals serve as reconciliation totals ONLY (`active == A+B`, `associated == A+C`, `entrants == A+B+C+D` — subtraction explicitly prohibited); and D005 decision IDs must equal the primary decision-ID set EXACTLY (duplicates/missing/extra fail closed). Attempt-1 counts are incident history only — never test oracles.
- **Secondary defect corrected (`D005_AUTHORIZED_STORE_HASH_PATH_YEAR_FALSE_POSITIVE`):** the D005 CLI now uses a structured pre-open boundary check (`validate_authorized_store_path`) — the authorized store basename `fold-01-a8b406884ab3525a` passes; holdout/final-validation tokens, standalone year components, `year=YYYY` markers, ISO `YYYY-` components (2025+), the prohibited historical store `fold-01-1d710826193a6767` and every other Fold basename are refused; hash/fingerprint fragments are never read as years; post-open structured store-identity checks remain authoritative. The committed CLI and the direct driver now execute the identical `run_d005`/`write_result` path; the untracked driver is no longer required for that reason and stays uncommitted.
- **Scope integrity:** spec §22 appended append-only (SHA `b098e3bf55b199a1f34b3ccb87c1d9cc14d58821b30832e8aaf2e0bfb65f8f11` bound in `SPEC_SHA256`; historical hashes `bab242de…`, `d7f39d31…`, `2b820d60…` preserved); register `result_records` closed with the byte-surgical TC002 record (+11,122 bytes, zero deletions); H003 statement byte-identical (status/result/disposition untouched); TC001 clock, 29-headroom arithmetic, attrition categories, primary-population definition, V002 strategy (blob `8272c285…`) and the V002 evaluator unchanged. D005 test suite: 47 passed + 2 opt-in skips; full repository suite green before the freeze commit. TC002 consumes ZERO budget; a corrected D005 rerun remains `4 / 12` (completes the already-consumed slot; must NOT become `5 / 12`) and awaits separate supervisory authorization.
