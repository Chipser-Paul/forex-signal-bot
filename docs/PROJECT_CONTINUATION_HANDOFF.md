...
## Update — 2026-09-25: D005 preregistered (Phase A) — temporal OB/FVG diagnostic — V003 deferred

- **State after V002 R001 review:** Tier B denied (`61 < 90`); downstream-only futility bounds recorded (`61+22=83`, `61+19=80`, both `< 90` — single-lever variants prohibited); D005 authorized as a READ-ONLY diagnostic. H003 compatibility gate passed before any record creation: the existing `phase8-v2-H003` is exactly the temporal/sequential OB↔FVG hypothesis and is preserved byte-identically (`SUPPORTED_BY_D003_D004_SYNTHESIS` H001, `SUPPORTED_BY_D003` H002, `SUPPORTED_BY_D004` H006 and V002 R001 all untouched).
- **D005 = `phase8-v2-D005`, REGISTERED_NOT_EXECUTED**, spec `docs/PHASE8_V2_DIAGNOSTIC_D005.md` (SHA `bab242de…`, bound in the register). Primary population: V002 Gate-11 entrants with `structurally_active == True` AND `fvg_associated == False` under frozen V002 semantics (R1/R2/R3 partition proof). Causal-only FVG history; final-FVG recomputation reconciled exactly against the persisted surface (fail closed); four exhaustive attrition categories; H003 temporal rule = ordering-based temporal sequence (same-direction FVG formed at/after OB confirmation; fill state descriptive only, never part of the rule); no proximity/lag/window, no temporal parameter search, counterfactual-candidate concepts banned fail-closed. **Correction (pre-empirical, disclosed):** the first Phase-A wording added a vacuous "still unfilled" clause to the rule (the persisted final surface is already unfilled-anywhere-in-frame, so the clause would force the temporal count to zero); corrected before tooling freeze — spec SHA now `d7f39d31…`, register entry corrected with the defect recorded inside it, H003 byte-identical. Necessary headroom arithmetic: `TEMPORAL_VARIANT_HEADROOM_POSSIBLE` iff `h003_temporal_association_decisions ≥ 29` (necessary condition only; H003 disposition follows H003's own preregistered rule, never the 29 rule).
- **Budget:** `3 / 12` diagnostics, `2 / 8` variants, `0 / 4` numeric, lock ACTIVE; D005 becomes `4 / 12` only when a separately authorized execution first reads Fold 01. `bot/strategy/variant_v002.py` stays byte-identical (blob `8272c285…`). Phase B (synthetic-tested tooling `backtests/phase8_v2_diagnostic_d005.py` + freeze commit) follows; HARD STOP before any empirical execution. V003 = NOT CREATED.

## Update — 2026-09-25: D005 tooling frozen (Phase B complete — awaiting supervisory review)

- **Frozen tooling identity:** `backtests/phase8_v2_diagnostic_d005.py` binds spec SHA `d7f39d31…` (the corrected ordering-only preregistration), the frozen V002 strategy module blob `8272c285…` (content-hashed via the canonical blob source at run time) and its own tooling fingerprint via `canonical_git_blob_v1`; result written non-overwriting as `phase8-v2-D005_result.json`. Read-only: reuses the frozen TC001/TC002 V002 observation path for the population and reconciles R1/R2/R3 partition, final-surface equality, block identity, causal boundaries and the exhaustive attrition categories fail-closed. 28 synthetic tests (`tests/test_phase8_v2_d005_tooling.py`) + full focused battery green before the freeze commit.
- **Next decision point (supervisory):** authorize D005 execution on `fold-01-a8b406884ab3525a` (diagnostics become permanently `4 / 12` at first observation) to measure `h003_temporal_association_decisions` against the 29-observation necessary headroom bound and apply H003's own preregistered qualitative rule; or close the cycle. Any temporal-association V003 remains UNGOVERNED until D005 evidence exists — no V003 has been created, and `>= 29` would be a necessary, never a sufficient, condition.

## Update — 2026-09-26: D005 TC001 — temporal clock aligned to causal availability (pre-empirical; awaiting supervisory review)

- **Defect class `D005_PRE_EMPIRICAL_TEMPORAL_CLOCK_MISMATCH` (register record `phase8-v2-D005-TC001`, byte-surgical additive append):** the frozen `81c6a6f` tooling compared the FVG completion candle's `open_time` against the OB `confirmed_at` (an `available_at` clock) — two different causal clocks; on the canonical contiguous M5 grid every FVG was reported exactly one bar early (true 0 → old −1; true +1 → old 0). **Corrected:** formation field is `fvg_formation_available_at` = completion candle's `available_at` (same clock as `block.confirmed_at`); the completion row remains `source_index + 1`; `fvg_completion_open_time` is descriptive-only metadata with proven zero consumers. No manual ±1 compensation exists anywhere.
- **Proof surface:** same-candle / next-candle / previous-candle regressions (verified against the real detectors), formation-field naming/causality, grid-invariant fail-closed, open-time descriptive invariance, and the in-process old-bug reproduction against the committed defective bytes (fixture `tests/fixtures/d005_tooling_81c6a6f.py`, SHA `b8bc31fa…`, with an opt-in byte-identity check). Spec §21 appended append-only (sections 1–20 untouched; prior hashes `bab242de848a…7401` and `d7f39d31…` preserved and Git-verified); corrected full-document SHA `2b820d60…` bound in `SPEC_SHA256` and the register. H003 byte-identical; ordering-only rule, primary population, 29-headroom arithmetic, final-surface reconciliation and attrition category definitions unchanged.
- **State:** budget `3 / 12`, `2 / 8`, `0 / 4`, lock ACTIVE; V003 NOT CREATED; zero Fold-01 reads; V002 module blob `8272c285…` and the V002 evaluator untouched. Next decision point (supervisory): authorize corrected-D005 execution on `fold-01-a8b406884ab3525a` (diagnostics become permanently `4 / 12` at first observation) or close the cycle. HARD STOP honored — D005 NOT executed.
## Update — 2026-09-26: D005 Attempt 1 blocked post-exposure; TC002 measurement correction frozen (awaiting supervisory rerun authorization)

- **Canonical state: diagnostics `4 / 12` PERMANENT (exposure `2026-09-26T13:18:42.145570+00:00`), variants `2 / 8`, numeric `0 / 4`, lock ACTIVE.** D005 Attempt 1 completed the full 13,269-snapshot Fold-01 observation loop with the frozen `7c9608d5` tooling, then failed closed in `aggregate_d005`: `ReconciliationError: primary population 42 != structurally-active without final FVG R2=34`. Nothing was sealed; the external artifact `C:/Users/chips/forex-signal-bot-data/phase8/v2_diagnostics/phase8-v2-D005/fold01/phase8-v2-D005_ATTEMPT1_BLOCKAGE.json` (SHA-256 `168ea3b27eb5cf536cca338bd87de028e2da2abd055ed112e85c508e1886f6e2`, 3,692 bytes) is the authoritative attempt-1 evidence and must stay unmodified. No successful D005 R001 exists; H003 remains REGISTERED with result/disposition null; V003 NOT created; Tier B sealed.
- **Root cause + correction (`phase8-v2-D005-TC002`, commit `fix: reconcile D005 V002 intersections exactly`, parent `7c9608d5…`):** `fvg_associated` is NOT a subset of `structurally_active` (real V002 can report MITIGATED + associated), so marginal subtraction was invalid. `aggregate_d005` now takes the per-decision `v002_observations` and builds the four-cell contingency (A/B/C/D; B = primary; R1 = A, R2 = B, R3 = C + D), reconciles the marginals as totals only (`active == A+B`, `associated == A+C`, `entrants == A+B+C+D`) and requires EXACT set equality between D005 decision IDs and the primary decision-ID set. The D005 CLI gained `validate_authorized_store_path` (structured pre-open boundary check; passes `fold-01-a8b406884ab3525a` whose `4068`/`3525` fragments are hash material; refuses holdout/2025+/historical/other-fold paths) — the committed CLI and direct driver are now equivalent, so the untracked `scripts/_d005_exec.py` driver is no longer required for that reason and must stay uncommitted. Spec §22 appended (full-doc SHA `b098e3bf55b199a1f34b3ccb87c1d9cc14d58821b30832e8aaf2e0bfb65f8f11` bound in `SPEC_SHA256`); attempt-1 counts (42/34/8) are incident history only and appear in no test assertion.
- **Next decision point (supervisory):** authorize the corrected D005 rerun — it completes the ALREADY-CONSUMED D005 slot and must remain `4 / 12` (never `5 / 12`); or close the cycle. On any rerun: expect the corrected aggregator to reconcile per-decision membership (the attempt-1 divergence is explained by `associated ⊄ active`); all other surfaces (TC001 clock, attrition categories, H003 ordering rule, 29-headroom arithmetic, distance surface) are unchanged and preregistered. Untracked helpers from this session (never commit): `scripts/_d005_preverify.py`, `scripts/_d005_fingerprint_probe.py`, `scripts/_d005_exec.py`, `scripts/_d005_blockage_seal.py`, `scripts/_d005_tc002_register.py`, `scripts/_tc002_progress_append.md`, `scripts/_tc002_handoff_append.md`.
